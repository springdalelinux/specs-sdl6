...
# with pvfs2?
# - no - seems to be broken for us in 1.5.*, due to a missing ADIOI_PVFS2_IReadContig
%if 0%{?suse_version}
...
%{!?compiler: %define compiler intel121}
...
#
# Intel 10.0 compiler
%if "%{compiler}" == "intel101" || "%{compiler}" == "intel111" || "%{compiler}" == "intel121"
...
#
# PGI compiler
%if "%{compiler}" == "pgi71" || "%{compiler}" == "pgi80" || "%{compiler}" == "pgi90" || "%{compiler}" == "pgi103" || "%{compiler}" == "pgi124"
...
# for pgi we need to just specify our compilers
%define compilerbuildprep export CC=pgcc CXX=pgcpp F77=pgfortran FC=pgfortran; . /etc/profile.d/modules.sh; module load pgi
%endif
...
Name: %{?_name:%{_name}}%{!?_name:openmpi}010405%{debug}-%{compiler}
Version: 1.4.5
%define numversion %(echo %{version} | tr -d .)
Release: 40.cses.5%{?dist}
License: BSD
...
%if %{pvfs2} && "%{?rhel}" < "5"
Patch5: openmpi-1.5.4-pvfsfix.patch
%endif
...
# compensate for icc miscompiling ptmalloc
%if "%{compiler}" == "intel121"
perl -pi -e 's|CC = icc|CC = gcc|' opal/mca/memory/Makefile
%endif
...
%if %{numversion} > 150
%{_prefix}/bin/ompi-probe
%{_prefix}/bin/ompi-profiler
%{_prefix}/bin/ompi-top
%{_prefix}/bin/orte-bootproxy.sh
%{_prefix}/bin/orte-top
%{_prefix}/bin/orteCC
%{_prefix}/bin/orte_wrapper_script
%{_prefix}/bin/ortec++
%{_prefix}/bin/ortecc
%endif
%{_prefix}/bin/orted
...
%{_prefix}/bin/vtCC
%{_prefix}/bin/vtc++
%{_prefix}/bin/vtunify-mpi
%{_prefix}/bin/vtwrapper
%endif
%{_prefix}/bin/vtcc
%{_prefix}/bin/vtcxx
%{_prefix}/bin/vtf77
...
%if %{numversion} > 150
%{_prefix}/bin/*otfprofile
%{_prefix}/bin/*otfshrink
...
#############################################################################
%changelog
* Mon Sep 26 2011 Josko Plazonic <plazonic@math.princeton.edu>
- upgrade to 1.5.4
