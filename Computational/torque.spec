...
%if %{use_uv}
%global uvflags     --enable-numa-support  --enable-memacct --enable-cpuset
%endif
...
Source0:     http://www.clusterresources.com/downloads/%{name}/%{name}-%{version}%{?snaprel}.tar.gz
...
Source8:     config
Patch11:      torque-billchanges.patch
Patch12:      torque-3.0.0-logpreempt.patch
...
%endif
%if %{use_uv}
BuildRequires: memacct libnuma libcpuset
Requires: memacct libnuma libcpuset
%endif
...
%prep
%setup -q -n torque-%{version}%{?snaprel}
%patch1 -p 1
%patch11 -p1 -b .billchanges
%patch12 -p1 -b .logpreempt
...
%configure --includedir=%{_includedir}/torque \
  ...
  --enable-drmaa \
  %{server_nameflags} %{guiflags} %{tclflags} %{rcpflags} %{uvflags}
...
%changelog
* Wed Feb 22 2012 Josko Plazonic <plazonic@math.princeton.edu>
- add support for SGI UV systems
